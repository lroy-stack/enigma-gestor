...
export function Sidebar({ isOpen, onClose, isPersistent = false, isCollapsed = false, onToggleCollapse }: SidebarProps) {
  ...
  return (
    <>
      {/* Overlay solo para modo no persistente - Bloqueo completo */}
      {!isPersistent && (
        <div 
          ...
          style={{
            ...
            width: '100vw',
            height: '100vh',
            touchAction: 'none', // Prevenir gestos táctiles
            overscrollBehavior: 'none' // Prevenir overscroll
          }}
          onClick={onClose}
          onTouchMove={(e) => e.preventDefault()} // Prevenir scroll táctil
        />
      )}

      {/* Sidebar ultra moderno con glassmorphismo */}
      <div 
        className={cn(
          ...
          // Visibilidad: persistente siempre visible, no persistente solo cuando isOpen
          isPersistent ? "translate-x-0" : (isOpen ? "translate-x-0" : "-translate-x-full")
        )}
        style={{
          touchAction: !isPersistent ? 'pan-y' : 'auto', // Solo permitir scroll vertical interno
        ...
        <div className="flex flex-col h-full relative">
          ...
          <div className="h-16 border-b border-white/10 backdrop-blur-xl relative overflow-hidden">
            ...
            <div className="flex items-center justify-between h-full px-6 relative z-10">
              {!isCollapsed && (
                <h2 className="text-lg font-bold bg-gradient-to-r from-enigma-primary to-enigma-secondary bg-clip-text text-transparent">
                  Menú
                </h2>
              )}
              {isPersistent && onToggleCollapse ? (
                <button
                  ...
                  className={cn(
                    "p-2 rounded-2xl hover:bg-white/10 transition-all duration-200 group",
                    isCollapsed && "mx-auto"
                  )}
              ...
              return (
                ...
                >
                  ...
                  <div className={cn(
                    ...
                    {/* Contenido del texto - solo cuando no está colapsado */}
                    {!isCollapsed && (
                      <>
          ...
          {/* Footer modernizado - solo cuando no está colapsado */}
          {!isCollapsed && (
            <div 
            ...
            >
              ...
              >
                ...
                <div className="text-center relative z-10">
                  <div 
                    className="w-14 h-14 rounded-2xl mx-auto mb-3 flex items-center justify-center shadow-lg relative overflow-hidden bg-white"
                    style={{ 
                  ...
                  <div className="text-xs text-gray-500 mt-1">
                    Versión 2.0.0 • Glassmorphismo iOS
                  </div>
                </div>
              </div>
            </div>
