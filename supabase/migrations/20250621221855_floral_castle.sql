/*
  ...
  1. New Tables
    - No se crean nuevas tablas, solo se modifican las existentes
  2. Changes
    - Añade tipo de notificación 'customer_modified'
    - Crea función para notificaciones de reservas actualizadas
    - Crea función para notificaciones de contactos modificados
    - Añade función para verificar reservas próximas
  3. Security
    - Otorga permisos de ejecución a usuarios autenticados
*/

-- Primero verificamos la estructura de notification_types para determinar el nombre de la columna de código
DO $$
DECLARE
    code_column_name TEXT;
BEGIN
    -- Comprobar si existe la columna 'code'
    SELECT column_name INTO code_column_name
    FROM information_schema.columns
    WHERE table_name = 'notification_types' AND column_name = 'code';
    
    -- Si no existe 'code', comprobar si existe 'type_code'
    IF code_column_name IS NULL THEN
        SELECT column_name INTO code_column_name
        FROM information_schema.columns
        WHERE table_name = 'notification_types' AND column_name = 'type_code';
    END IF;
    
    -- Si no existe ninguna, usar 'name' como identificador único
    IF code_column_name IS NULL THEN
        code_column_name := 'name';
    END IF;
    
    -- Insertar el tipo de notificación 'customer_modified' usando la columna correcta
    IF code_column_name = 'code' THEN
        EXECUTE 'INSERT INTO notification_types (code, name, description, icon_name, color) VALUES
            (''customer_modified'', ''Contacto Modificado'', ''Información de un contacto ha sido actualizada'', ''user-check'', ''#237584'')
            ON CONFLICT (code) DO NOTHING;';
    ELSIF code_column_name = 'type_code' THEN
        EXECUTE 'INSERT INTO notification_types (type_code, name, description, icon_name, color) VALUES
            (''customer_modified'', ''Contacto Modificado'', ''Información de un contacto ha sido actualizada'', ''user-check'', ''#237584'')
            ON CONFLICT (type_code) DO NOTHING;';
    ELSE
        EXECUTE 'INSERT INTO notification_types (name, description, icon_name, color) VALUES
            (''customer_modified'', ''Información de un contacto ha sido actualizada'', ''user-check'', ''#237584'')
    ...
    IF OLD.estado IS DISTINCT FROM NEW.estado THEN
        ...
        CASE NEW.estado
            WHEN 'confirmada' THEN
                ...
                v_type_code := 'reservation_confirmed';
            WHEN 'cancelada' THEN
...
-- Comentario regular en lugar de COMMENT ON MIGRATION
-- Enhances notification system with triggers for reservation and contact modifications, 
-- time-based upcoming reservation checks, and duplicate cleanup